...
function toMySqlUtcDateTime(value: Date): string {
  return value.toISOString().slice(0, 19).replace('T', ' ');
}

function coerceToUtcDate(value: Date | string): Date {
...
export class GitHubService {
  ...
  async verifyInstallationForRepo(
    installationId: string,
    owner: string,
    repo: string
  ): Promise<{ matches: boolean; expectedInstallationId?: number; actualInstallationId?: number }> {
    const installation = await query<any[]>(
      'SELECT installation_id FROM github_installations WHERE id = ?',
      [installationId]
    );
    ...
    const expectedInstallationId = Number(installation[0].installation_id);

    const { data } = await this.app.apps.getRepoInstallation({ owner, repo });
    const actualInstallationId = Number(data.id);

    return {
      matches: expectedInstallationId === actualInstallationId,
  ...
  private async cacheToken(
    ...
    const mysqlDateTime = toMySqlUtcDateTime(new Date(expiresAt));

    await query(
      `DELETE FROM github_access_tokens WHERE installation_id = ?`,
      [installationId]
    );
    ...
    await query(
      `INSERT INTO github_access_tokens (id, installation_id, token_encrypted, expires_at)
       VALUES (?, ?, ?, ?)`,
      [id, installationId, encrypted, mysqlDateTime]
    );
  }
  ...
  private async getCachedToken(
    ...
    return {
      ...
      expiresAt: coerceToUtcDate(results[0].expires_at),
    };
  }
}
