...
async function handleInstallation(payload: any) {
  ...
  if (action === 'created') {
    ...
    // First get the internal installation ID
...
async function handlePush(payload: any) {
  ...
  if (ref !== defaultRef) {
    console.log(`Push to non-default branch ${ref}, marked as stale`);
  } else {
    console.log(`Repository ${repository.full_name} marked as stale`);
  }
}
